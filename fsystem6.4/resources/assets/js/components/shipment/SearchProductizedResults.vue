<template>
  <table class="table table-color-bordered">
    <colgroup>
      <col class="col-md-2 col-sm-2 col-xs-2">
      <col class="col-md-4 col-sm-4 col-xs-4">
      <col class="col-md-2 col-sm-2 col-xs-2">
      <col class="col-md-4 col-sm-4 col-xs-4">
    </colgroup>
    <tbody>
      <tr>
        <th>工場<span class="required-mark">*</span></th>
        <td>
          <select
            id="factory_code"
            name="factory_code"
            class="form-control"
            :class="{'has-error': 'factory_code' in errors}"
            v-model="factoryCode"
            @change="getSpeciesWithFactoryCode">
            <option value=""></option>
            <option v-for="f in factories" :key="f.factory_code" :value="f.factory_code">{{ f.factory_abbreviation }}</option>
          </select>
        </td>
        <th>品種</th>
        <td>
          <select
            id="species_code"
            name="species_code"
            class="form-control"
            :class="{'has-error': 'species_code' in errors}"
            v-model="speciesCode" :disabled="disabledToSelectSpecies">
            <option value=""></option>
            <option v-for="s in speciesList" :key="s.species_code" :value="s.species_code">{{ s.species_name }}</option>
          </select>
        </td>
      </tr>
      <tr>
        <th>収穫日</th>
        <td>
          <datepicker-ja attr-name="harvesting_date" :date="harvestingDate" :allow-empty="true">
          </datepicker-ja>
        </td>
      </tr>
    </tbody>
  </table>
</template>

<script>
export default {
  props: {
    factories: {
      type: Array,
      required: true
    },
    searchParams: {
      type: Object,
      required: true
    },
    oldParams: {
      type: Object,
      required: true
    },
    errors: {
      type: [Array, Object],
      required: true
    }
  },
  data: function () {
    return {
      factoryCode: this.oldParams.factory_code || this.searchParams.factory_code || '',
      speciesCode: this.oldParams.species_code || this.searchParams.species_code || '',
      speciesList: [],
      harvestingDate: this.oldParams.harvesting_date || this.searchParams.harvesting_date || ''
    }
  },
  created: function () {
    if (this.factoryCode) {
      this.getSpeciesWithFactoryCode()
      this.speciesCode = this.oldParams.species_code || this.searchParams.species_code
    }
  },
  computed: {
    disabledToSelectSpecies: function () {
      return this.speciesList.length === 0
    }
  },
  methods: {
    getSpeciesWithFactoryCode: function () {
      this.speciesCode = ''
      this.speciesList = []
      if (! this.factoryCode) {
        return
      }

      axios.get('/api/get-species-with-factory-code', {
        params: {
          factory_code: this.factoryCode
        }
      })
        .then(response => {
          this.speciesList = response.data
          if (this.disabledToSelectSpecies) {
            alert('工場取扱品種が未登録の工場です。')
          }
        })
        .catch(() => {
          alert('通信エラーが発生しました。しばらくお待ちください。')
        })
    }
  }
}
</script>
