...
<div class="row">
  ...
  <form action="{{ route('plan.growth_simulation.add.search') }}" method="POST">
    <div class="col-md-8 col-sm-6 col-xs-8 col-md-offset-1 col-sm-offset-1">
      <select-form-to-add-simulation
        :factories="{{ $factories }}"
...
<start-simulation-form
  :factory="{{ $factory }}"
  :factory-species="{{ $factory_species }}"
  :factory-growing-stages="{{ $factory_species->factory_growing_stages }}"
  :max-pattern="{{ json_encode(config('settings.data_link.plan.growth_simulation.max_pattern')) }}"
  ...
  action-of-creating-simulation="{{ route('plan.growth_simulation.create') }}"
