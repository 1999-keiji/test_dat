    ...
    <tbody>
      ...
      <tr>
        ...
        @foreach ($harvesting_date->toListOfDate($delivery_date->getWeekTermOfExportOrderForecast()) as $hd)
          ...
          <td align="right" style="color: #FF0000; background-color: #F2DCDB; border-bottom: 2px thick #000;">
            {{ $hd->formatShortWithDayOfWeek() }}
          </td>
          @else
      ...
      <tr>
        <td></td>
        <td style="border-bottom: 1px double #000; border-right: 0px none #000;"></td>
        <td style="border-bottom: 1px double #000; border-right: 0px none #000;">{{ __('view.plan.growth_sale_management.product_weight') }}（kg）</td>
        <td style="border-bottom: 1px double #000; border-right: 0px none #000;"></td>
        <td style="border-bottom: 1px double #000; border-right: 0px none #000;"></td>
        <td style="border-bottom: 1px double #000; border-right: 0px none #000;"></td>
      ...
      <tr>
        ...
        <td
          ...
          data-format="#,##0.00">
          =
          @foreach (range(0, ($shipment_lead_time->getMaximumShipmentLeadTime() + $delivery_lead_time->getMaximumNum())) as $lt)
            SUMIF(
              F{{ $config['visible_forecast']['merge_start_row'] }}:
              F{{ $end_row }},
              "={{ $lt }}",
              {{ get_excel_column_str($idx + $config['visible_forecast']['date_start_column'] + $lt).$config['visible_forecast']['merge_start_row'] }}:
              {{ get_excel_column_str($idx + $config['visible_forecast']['date_start_column'] + $lt).$end_row }}
            )
            @if (! $loop->last)
            +
            @endif
      ...
      <tr>
        ...
        @foreach ($harvesting_date->toListOfDate($delivery_date->getWeekTermOfExportOrderForecast()) as $idx => $hd)
        <td
          style="border-bottom: 1px double #000;
          @if (! $hd->isWorkingDay($factory))
      ...
      <tr>
        ...
        <td></td>
        @foreach ($delivery_dates as $idx => $dd)
        <td data-format="#,##0.00">
          =SUM(
            {{ get_excel_column_str($idx + $config['visible_forecast']['date_start_column']).$config['visible_forecast']['merge_start_row'] }}:
            {{ get_excel_column_str($idx + $config['visible_forecast']['date_start_column']).$end_row }}
          )
        </td>
        @endforeach
      </tr>
      ...
      @foreach ($factory_products as $fp)
        ...
        <tr>
          ...
          @foreach ($dd->delivery_dates as $idx => $delivery_date)
            <td @if (! $delivery_date['can_import']) style="background-color: #D3D3D3;" @endif data-format="#,##0.00">
              ={{ $config['visible_forecast']['sheet_name_number'] }}!{{ get_excel_column_str($idx + $config['visible_forecast']['date_start_column']).$row }}
              *{{ $fp->weight_per_number_of_heads }}*{{ $fp->number_of_cases }}/1000
            </td>
          @endforeach
        </tr>
        @php ($row = $row + 1)
