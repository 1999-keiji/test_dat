  ...
  <div class="col-md-5 col-sm-5 col-xs-6 col-md-offset-1 col-sm-offset-1">
    <div class="row form-group">
      ...
      <div class="col-md-7 col-sm-7">
        <textarea id="remark" class="form-control {{ has_error('remark') }}" name="remark" rows="3" {{ $master->addDisabledProp('remark') }}>{{ old('remark', $master->remark) }}</textarea>
      </div>
    </div>
  </div>
</div>
...
<div class="row">
  <div class="col-md-5 col-sm-5 col-xs-6 col-md-offset-1 col-sm-offset-1">
    <div class="row form-group">
      ...
      <div class="col-md-7 col-sm-7">
        <input id="reserved_text{{ $idx }}" class="form-control {{ has_error("reserved_text{$idx}") }}" maxlength="200" name="reserved_text{{ $idx }}" type="text" value="{{ old("reserved_text{$idx}", $master->{"reserved_text$idx"}) }}" {{ $master->addDisabledProp("reserved_text{$idx}") }}>
      </div>
    </div>
  </div>
  @php
    ...
    <div class="row form-group">
      ...
      <div class="col-md-7 col-sm-7">
        <input-number-with-formatter
          ...
          value="{{ old("reserved_number{$idx}", $master->{"reserved_number$idx"}) }}"
          decimals="5"
          is-invalid="{{ has_error("reserved_number{$idx}") !== '' ? 1 : 0 }}"
          is-disabled="{{ $master->addDisabledProp("reserved_number{$idx}") !== '' ? 1 : 0 }}">
        </input-number-with-formatter>
      </div>
    </div>
  </div>
