          ...
          @foreach ($factory_products as $fp)
            <tr>
              <td>
                ...
                @if ($factory_product->sequence_number === $fp->sequence_number)
                checked
                @endif>
              </td>
...
<edit-factory-product
  action-of-update-factory-product="{{ route('master.factory_products.update', [$factory->factory_code, $factory_product->getJoinedPrimaryKeys()]) }}"
  action-of-delete-factory-product="{{ route('master.factory_products.delete', [$factory->factory_code, $factory_product->getJoinedPrimaryKeys()]) }}"
  :factory-product="{{ $factory_product }}"
  :is-deletable="{{ json_encode($factory_product->isDeletable()) }}"
  :species="{{ $factory_product->product->species }}"
  :product="{{ $factory_product->product }}"
  :current-factory-product-prices="{{ $factory_product->factory_product_prices }}"
