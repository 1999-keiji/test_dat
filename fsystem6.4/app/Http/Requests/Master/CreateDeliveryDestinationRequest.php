...
class CreateDeliveryDestinationRequest extends FormRequest
{
    /**
     ...
     * @param  \App\ValueObjects\String\DeliveryDestinationCode
    ...
    public function __construct(
        ...
        EndUser $end_user,
        DeliveryDestinationCode $delivery_destination_code,
    ...
    public function rules()
    {
        $rules = [
            'delivery_destination_code' => [
                'bail',
                'required',
                "unique:{$this->delivery_destination->getTable()}",
                "min:{$this->delivery_destination_code->getMinLength()}",
                "max:{$this->delivery_destination_code->getMaxLength()}",
                "regex:{$this->delivery_destination_code->getRegexPattern()}"
            ],
            'delivery_destination_name' => ['bail', 'required', 'string', 'max:50'],
            'delivery_destination_name2' => ['bail', 'nullable', 'string', 'max:50'],
            'delivery_destination_abbreviation' => ['bail', 'required', 'string', 'max:20'],
            'delivery_destination_name_kana' => ['bail', 'required', 'max:30', 'hankana'],
            'delivery_destination_name_english' => ['bail', 'nullable', 'max:65', 'alpha_period_dash'],
            'country_code' => [
                'bail',
                'required',
                'string',
            ...
            'shipping_label_unnecessary_flag' => ['bail', 'required', 'boolean'],
