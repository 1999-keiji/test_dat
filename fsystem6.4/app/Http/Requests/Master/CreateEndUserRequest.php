...
class CreateEndUserRequest extends FormRequest
{
    /**
    ...
    public function __construct(
        ...
        $this->on_off_checkboxes = $this->end_user->getWillCastAsBoolean();
    }
    ...
    public function rules()
    {
        $rules = [
            'end_user_code' => [
                'bail',
                'required',
                Rule::unique($this->end_user->getTable())->where(function ($query) {
                    return $query->where('application_started_on', $this->application_started_on);
                }),
            ...
            'end_user_name' => ['bail', 'required', 'string', 'max:50'],
            'end_user_name2' => ['bail', 'nullable', 'string', 'max:50'],
            'end_user_abbreviation' => ['bail', 'required', 'string', 'max:20'],
            'end_user_name_kana' => ['bail', 'required', 'string', 'max:30', 'hankana'],
            'end_user_name_english' => ['bail', 'nullable', 'string', 'max:65', 'alpha_period_dash'],
            'country_code' => [
                'bail',
                'required',
                'string',
            ...
            'phone_number' => ['bail', 'required', 'max:20', 'regex:/\A[0-9-]+\z/'],
            ...
            'seller_name' => ['bail', 'required', 'max:30'],
            ...
            'lot_managing_target_flag' => ['bail', 'required', 'boolean'],
            ...
            'group_company_flag' => ['bail', 'required', 'boolean'],
    ...
    {
        return [
            'end_user_code.unique' => '指定されたエンドユーザコードと適用開始日の組み合わせのマスタはすでに登録されています。'
        ];
    }
}
