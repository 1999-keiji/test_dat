...
class CreateArrangementDetailStatesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('arrangement_detail_states', function (Blueprint $table) {
            ...
            for ($i = 1; $i <= 100; $i++) {
                $table->unsignedTinyInteger('panel_status_'.$i)->nullable()->comment('パネル状況'.$i);
            }
            ...
            $table->primary([
                ...
                'bed_row',
                'bed_column'
            ], 'arrangement_detail_states_primary_key');

            $table->foreign([
                'factory_code',
                'factory_species_code',
                'start_of_week'
            ], 'bed_states_foreign_arrangement_detail_states')
    ...
    {
        Schema::dropIfExists('arrangement_detail_states');
    }
}
